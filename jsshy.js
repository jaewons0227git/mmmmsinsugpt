...
let currentLoadingText = '대답을 생각하고 있어';
...
function toggleImageMode(active) {
    ...
    if (active) {
        ...
        currentLoadingText = '대답을 생각하고 있어';
        if (isSimple) {
...
function openDeleteModal(type, id = null) {
    ...
    if (type === 'all') {
        ...
        deleteModalDesc.textContent = "진짜 삭제할꺼야??";
    } else {
...
function handleRegenerate(messageIndex) {
    ...
    if (modelMessageIndex === -1) { showSnackbar('재생성할 답변을 찾을 수 없어!'); return; }
    ...
    currentLoadingText = '다시 대답을 생각하고 있어';
    ...
    try {
        if (isImageMode) {
            ...
            if (data.success && data.image_data) {
                ...
                throw new Error(data.error || "이미지 생성에 실패했어 ㅠㅠ");
            }
        ...
        if (error.name === 'AbortError') { 
            ...
            const errorMsg = `⚠️ 오류가 발생했어 ㅠㅠ ${error.message}`;
